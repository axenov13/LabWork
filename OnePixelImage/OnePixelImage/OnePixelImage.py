...
def get_signal1(ser, n):
    return random()*100
def getAlanDev1(ser, n):
    dev = 0
    x1 = get_signal1(ser, 1)
    for i in range(n):
        x2 = get_signal1(ser, 1)
        sleep(0.01)
        dev += (x2 - x1)*(x2 - x1)/2
        x1 = x2
    return float(dev)/n/n/n
def scanBrick1(ser, canv, bricksize, number, diod_reaction_time):
    width = bricksize
    height = bricksize
    list = []
...
list = scanBrick1(ser, w, brick, num, waitfor)

noize = get_signal1(ser, num)
dev = getAlanDev1(ser, num)
print noize, dev
